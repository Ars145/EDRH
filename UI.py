...
def detect_commander_renames(journal_path):
    ...
    try:
        journal_files = [f for f in os.listdir(journal_path) if f.startswith("Journal.") and f.endswith(".log")]
        ...
        for journal_file in journal_files:
            ...
            try:
                ...
                file_date = timestamp_match.group(1) if timestamp_match else "Unknown"
...
def find_latest_journal_and_pos_static(fp):
    """Static version of find_latest_journal_and_pos for use before class init"""
        ...
        with open(fp, encoding="utf-8") as f:
            for line in f:
                if '"event":"Commander"' in line:
                    m = re.search(r'"Name"\s*:\s*"([^"]+)"', line)
                    if m:
                        last_commander = m.group(1)
                elif '"event":"LoadGame"' in line:
                    m = re.search(r'"Commander"\s*:\s*"([^"]+)"', line)
                    if m:
                        last_commander = m.group(1)

        if last_commander == "Unknown":
            ...
            most_recent_cmdr = None

            for journal_file in journal_files[:20]:
                    ...
                    with open(journal_path, encoding="utf-8") as f:
                        for line in f:
                            if '"event":"LoadGame"' in line and '"Commander"' in line:
                                ...
                                if timestamp_match and cmdr_match:
                                    timestamp = timestamp_match.group(1)
                                    commander = cmdr_match.group(1)

                                    if most_recent_time is None or timestamp > most_recent_time:
                                        most_recent_time = timestamp
                                        most_recent_cmdr = commander
                except:
                    continue

            if most_recent_cmdr:
                last_commander = most_recent_cmdr
                print(f"[DEBUG] Using most recent commander '{last_commander}' from LoadGame at {most_recent_time}")

        return last_commander
    except Exception as e:
        print(f"[ERROR] Failed to extract commander name: {e}")
    return "Unknown"
    ...
    for i, path in enumerate(potential_paths, 1):
        try:
            ...
            if os.path.exists(path):
                journal_files = [f for f in os.listdir(path) if f.startswith("Journal.") and f.endswith(".log")]
                if journal_files:
                    print(f"[AUTO-DETECT] ✓ SUCCESS! Found {len(journal_files)} journal files at: {path}")
                    return path
                else:
                    print(f"[AUTO-DETECT] Path exists but no journal files found")
            else:
...
def analyze_journal_folder(folder_path):
    ...
    journal_files = [f for f in os.listdir(folder_path) if f.startswith("Journal.") and f.endswith(".log")]
    if not journal_files:
        return None

    journal_files.sort(reverse=True)
    print(f"[ANALYSIS] Found {len(journal_files)} journal files")
...
class ZoomableMap(ctk.CTkToplevel):
    ...
    def toggle_done_systems(self):
        if self.cb_done.get() and supabase:
            ...
            if "All Categories" not in self.selected_categories and self.selected_categories:
                systems_data = [s for s in systems_data if s.get("category") in self.selected_categories]
    ...
    def toggle_unvisited(self):
        if self.cb_unv.get() and supabase:
            ...
            if "All Categories" in self.selected_categories or not self.selected_categories:
                ...
                pass
            if "All Categories" not in self.selected_categories and self.selected_categories:
                all_sys = [s for s in all_sys if s.get("category") in self.selected_categories]
        ...
        if self.cb_you.get() and supabase:
            ...
            if names:
                ...
                try:
                    ...
                    pass
                data = [s for s in data if s.get("category") not in richards_categories]
                if "All Categories" not in self.selected_categories and self.selected_categories:
                    data = [s for s in data if s.get("category") in self.selected_categories]
                for rec in data:
                    rec["by_cmdr"] = cmdr
                self.you_data = data
            else:
    ...
    def setup_category_dropdown(self):
        ...
        if hasattr(scroll_frame, '_parent_canvas'):
            scroll_frame._parent_canvas.bind("<MouseWheel>", lambda e: scroll_frame._parent_canvas.yview_scroll(int(-1*(e.delta/120)), "units"), add="+")
...
class App(ctk.CTk):
    ...
    def get_category_table_data(self, system_name, category):
        ...
        try:
            ...
            print(f"Looking for table: '{table_name}' for system: '{system_name}'")
            escaped_table = table_name.replace('"', '""')
            response = supabase.table(table_name).select("*").execute()
            print(f"Table response: {response.data[:2] if response.data else 'No data'}")
            ...
            for row in response.data:
                ...
                for col_name in ['system', 'System', 'SYSTEM', 'systems', 'Systems', 'SYSTEMS']:
                    if col_name in row:
                        ...
                        print(f"Found system column '{col_name}' with value: '{system_value}'")
                        break
                if system_value and system_value.lower().strip() == system_name.lower().strip():
                    print(f"Found matching system!")
                    info_parts = []
                    ...
                    if info_parts:
                        result = '\n'.join(info_parts)
                        print(f"Returning info: {result}")
                        return result
                    return None
            print(f"No matching system found in table")
            return None
        except Exception as e:
        ...
        def load_systems():
            try:
                ...
                if "All Categories" not in self.selected_categories_main and self.selected_categories_main:
                    all_systems = [s for s in all_systems if s.get("category") in self.selected_categories_main]
    ...
    def _create_system_card(self, sys, poi_data, done_data, category_images, system_images):
        try:
            ...
            category_text = sys['category']
            if len(category_text) > 45:
                ...
                for word in words:
                    if len(line1 + word) < 40:
                        ...
                        line2 += word + " "
                canvas.create_text(15, 50, text=line1.strip(),
                                  font=("Arial", 10),
                                  fill="#CCCCCC",
                                  anchor="w")
                if line2:
                    canvas.create_text(15, 65, text=line2.strip(),
                                      font=("Arial", 10),
                                      fill="#CCCCCC",
                                      anchor="w")
            else:
                canvas.create_text(15, 50, text=category_text,
                                  font=("Arial", 11),
                                  fill="#CCCCCC",
                                  anchor="w")
    ...
    def find_nearest_unclaimed(self):
        ...
        try:
            ...
            try:
                ...
                pass
            if "All Categories" in self.selected_categories_main or not self.selected_categories_main:
                systems = [s for s in systems if s.get("category") not in richards_categories]
            else:
                pass
            if "All Categories" not in self.selected_categories_main and self.selected_categories_main:
                systems = [s for s in systems if s.get("category") in self.selected_categories_main]
            taken = {r["system"] for r in supabase.table("taken").select("system").execute().data or []}
            pois = supabase.table("pois").select("system_name").execute().data or []
            poi_systems = {poi["system_name"] for poi in pois}
    ...
    def _build_system_popup_content(self, popup, system_name, category):
        ...
        if system_in_database and systems_check.data:
            category_text = systems_check.data[0].get("category", "Unknown Category")
            ctk.CTkLabel(info_content, text=category_text,
                        font=ctk.CTkFont(size=14),
        ...
        if is_visited or poi_data:
            ...
            if poi_data:
                ...
                if system_info and system_info.get("images"):
                    all_images.append(("System Image", system_info.get("images")))
                elif preset_img_url and not poi_data:
                    all_images.append(("Example Image", preset_img_url))
                if system_info and system_info.get("additional_images"):
                    try:
                        additional_imgs = json.loads(system_info["additional_images"])
...
or via public URLs will be visible to other users."""
                    ...
                    if not system_in_database and cat_dropdown:
                        ...
                        if self.system_name == system_name and self.latest_starpos:
                            ...
                            new_system = {
                                ...
                                "y": y,
                                "z": z,
                                "category": updates.get("category", "POI System")
                            }
                            try:
                ...
                if hasattr(popup, '_system_images_scroll') and widget_under_mouse:
                    if _is_widget_in_area(widget_under_mouse, popup._system_images_scroll):
                        if hasattr(popup._system_images_scroll, '_parent_canvas'):
                            popup._system_images_scroll._parent_canvas.yview_scroll(int(-12*(event.delta/120)), "units")
                        return "break"
                ...
                if hasattr(popup, '_poi_images_scroll') and widget_under_mouse:
                    if _is_widget_in_area(widget_under_mouse, popup._poi_images_scroll):
                        if hasattr(popup._poi_images_scroll, '_parent_canvas'):
                            popup._poi_images_scroll._parent_canvas.yview_scroll(int(-12*(event.delta/120)), "units")
                        return "break"
    ...
    def ask_for_journal_folder(self):
        while True:
            ...
            journal_files = [f for f in os.listdir(fld) if f.startswith("Journal.") and f.endswith(".log")]
            if journal_files:
